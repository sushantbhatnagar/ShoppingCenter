...
class BrowserEngine
  ...
  def chrome_browser
    ...
    if @world.configuration['USE_GRID']
      driver = Selenium::WebDriver.for(:remote, :url => @world.configuration['ENVIRONMENT']['GRID'] , desired_capabilities: caps, switches: ["no-sandbox"])
    else
      if OS.linux?
      ...
      driver = Selenium::WebDriver.for(:chrome, desired_capabilities: caps, driver_path: path, switches: ["no-sandbox"])
    end
